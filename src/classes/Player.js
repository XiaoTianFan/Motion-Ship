class Player {
  constructor(model, texture) { 
    ...
    this.colliderRadius = 30; // Example radius for collision detection
  ...
  update(headPos) {
    if (this.toDestroy === false) {
      ...
      let targetX = map(headPos.x, -1, 1, -width * 1.5, width * 1.5);
      let targetY = map(headPos.y, -1, 1, -height * 2, height * 2);
